...
class FA2_core(sp.Contract):
    ...
    @sp.onchain_view()
    def balance_of_view(self, req):
        # paused may mean that balances are meaningless:
        sp.verify( ~self.is_paused(), message = self.error_message.paused())
        sp.set_type(req, sp.TRecord(owner = sp.TAddress,token_id = sp.TNat).layout(("owner", "token_id")))
        user = self.ledger_key.make(req.owner, req.token_id)
        sp.verify(self.data.token_metadata.contains(req.token_id), message = self.error_message.token_undefined())
        sp.if self.data.ledger.contains(user):
            balance = self.data.ledger[user].balance
...
class FA2_administrator(FA2_core):
    ...
    @sp.entry_point
    def set_minter1(self, params):
        sp.verify(sp.sender == self.data.minter1, message = self.error_message.not_minter())
        self.data.minter1 = params
...
class FA2_mint(FA2_core):
    ...
    def tokens(self, params):
        ...
        with params.match_cases() as arg:
            with arg.match("mint_tokens") as upd:
                sp.for mint_tokens_item in upd:
            ...
            with arg.match("burn_tokens") as upd:
                sp.for burn_tokens_item in upd:
...
class FA2(FA2_change_metadata, FA2_token_metadata, FA2_mint, FA2_administrator, FA2_pause, FA2_core):
    ...
    def __init__(self, config, metadata, admin, minter1, minter2):
